...
export default function Navbar({
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between h-16 sm:h-20 gap-2 sm:gap-4">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
              ...
              {/* Currency Selector Pill (Opens clean responsive modal) */}
              <button
                onClick={() => setIsCurrencyModalOpen(true)}
                className="flex items-center gap-1 py-1.5 px-2.5 sm:px-3 rounded-xl bg-slate-100 hover:bg-slate-200/80 text-xs font-bold text-slate-700 transition cursor-pointer border border-slate-200/60"
                title="Change Country & Currency"
              >
                <span className="text-sm">{currentCurrencyConfig.flag}</span>
                <span className="font-mono font-extrabold text-xs">{currentCurrencyConfig.code} ({currentCurrencyConfig.symbol})</span>
              ...
              <button
                ...
                className="md:hidden p-2 rounded-xl text-slate-700 hover:bg-slate-100 border border-slate-200/80 transition cursor-pointer"
      ...
      {/* 1. CLEAN RESPONSIVE CURRENCY SELECTOR MODAL */}
      {isCurrencyModalOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
          <div 
            className="bg-white w-full max-w-md rounded-t-3xl sm:rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh] animate-in slide-in-from-bottom-10 sm:zoom-in-95 duration-200 pb-safe sm:pb-0"
          ...
          >
            ...
            <div className="p-4 sm:p-5 border-b border-slate-200 flex items-center justify-between bg-slate-50 shrink-0">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-sm sm:text-base font-extrabold text-slate-900">Select Region & Currency</h3>
                  <p className="text-[10px] sm:text-xs text-slate-500">Prices will convert automatically</p>
                </div>
              </div>
              <button 
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <span className="text-xs sm:text-sm font-extrabold text-slate-900 block">{c.name}</span>
                        <span className="text-[11px] text-slate-500 font-mono">{c.code}</span>
                      </div>
                    </div>

                    <div className="flex items-center gap-2">
                      <span className="font-mono font-extrabold text-sm sm:text-base text-slate-800 bg-slate-100 px-2.5 py-1 rounded-xl">
                        {c.symbol}
                      </span>
                      {isSelected && (
      ...
      {/* 2. SLIDE-OUT MOBILE DRAWER MENU (Properly Sized & Formatted) */}
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-50 overflow-hidden md:hidden animate-in fade-in duration-200">
          ...
          {/* Drawer container: exactly 85vw width (max 360px), perfectly proportioned */}
          <div className="fixed inset-y-0 right-0 max-w-full flex">
            <div className="w-[85vw] max-w-xs sm:max-w-sm bg-white shadow-2xl flex flex-col justify-between animate-in slide-in-from-right duration-250 pb-safe">
              ...
              <div className="p-4 border-b border-slate-200 flex items-center justify-between bg-slate-50 shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-extrabold text-slate-900 text-sm sm:text-base">foodRo Menu</span>
                </div>
                <button
                  onClick={() => setIsMobileMenuOpen(false)}
                  className="w-8 h-8 rounded-full hover:bg-slate-200 flex items-center justify-center text-slate-600 transition cursor-pointer"
                  ...
                  <div className="p-3.5 rounded-2xl bg-orange-50 border border-orange-200">
                    <div className="flex items-center gap-3">
                      <img
                        src={user.avatar}
                        alt={user.name}
                        className="w-10 h-10 rounded-2xl object-cover border border-orange-500 shrink-0"
                      />
                      <div className="min-w-0">
                        ...
                        <span className="text-[9px] font-bold text-emerald-700 bg-emerald-100 px-1.5 py-0.5 rounded-full mt-0.5 inline-flex items-center gap-0.5">
                          <ShieldCheck className="w-2.5 h-2.5" />
                          2FA Verified
                        </span>
                      </div>
                    ...
                    <button
                      ...
                      className="w-full mt-2.5 py-1.5 text-xs font-bold text-rose-600 bg-white border border-rose-200 rounded-xl flex items-center justify-center gap-1.5 cursor-pointer"
                    >
                ...
                <div className="space-y-1 pt-1">
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      <span className="text-base">{currentCurrencyConfig.flag}</span>
                      <span>Currency: {currentCurrencyConfig.code} ({currentCurrencyConfig.symbol})</span>
                    </div>
                    <span className="text-[10px] bg-slate-100 text-slate-700 font-extrabold px-2 py-0.5 rounded-full">Change</span>
                  </button>
              ...
              {/* Drawer Footer */}
              <div className="p-3.5 bg-slate-50 border-t border-slate-200 text-[10px] text-slate-400 text-center shrink-0">
